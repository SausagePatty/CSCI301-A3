...
class MySubscribeCallback(SubscribeCallback):
    ...
    def message(self, pubnub, message):
        if 'from' in message.message and message.message['from'] == 'Bob':
...
def mine_block(block_number):
    ...
    while True:
        ...
        if int(block_hash, 16) < 2 ** 236:
            ...
            print("Block mined by Alice:", block)
            os.makedirs("AliceBlocks", exist_ok=True)
            with open(f"AliceBlocks/block_{block_number}.json", 'w') as f:
                json.dump(block, f)
            pubnub.publish().channel('channel-bob').message(block).pn_async(my_publish_callback)
...
def main():
    global nonce, transactions
    transactions = [
        None,  # Placeholder for the 0th block
        "[3, 4, 5, 6]", 
        "[4, 5, 6, 7]",
        "[5, 6, 7, 8]",  
        "[6, 7, 8, 9]",  
    ...
    print("Alice is mining...")
    for i in range(1, len(transactions)):
        if i % 2 != 0:  # Alice generates blocks for odd block numbers
            alice_thread = threading.Thread(target=mine_block, args=(i,))
            alice_thread.start()
